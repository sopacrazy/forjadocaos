...
import { Scroll, Clock, Coins, Star, Swords, CheckCircle, XCircle, AlertCircle } from 'lucide-react';
...
const MissionsBoard: React.FC<MissionsBoardProps> = ({ currentUser, onUpdateUser }) => {
    ...
    // Carregar missões
    useEffect(() => {
        const savedMissions = localStorage.getItem(`rpg_missions_${currentUser.email}`);
        if (savedMissions) {
    ...
    const saveMissions = (updatedMissions: Mission[]) => {
        ...
        localStorage.setItem(`rpg_missions_${currentUser.email}`, JSON.stringify(updatedMissions));
    };
    ...
    const completeMission = (mission: Mission) => {
        ...
        const updatedUser = {
            ...currentUser,
            xpAtual: (currentUser.xpAtual || 0) + mission.xpReward,
            dinheiro: (currentUser.dinheiro || 0) + mission.goldReward
        };

        // Verificar level up (simples)
        if (updatedUser.xpAtual >= (updatedUser.xpProximo || 1000)) {
            updatedUser.nivel = (updatedUser.nivel || 1) + 1;
            updatedUser.xpAtual = updatedUser.xpAtual - (updatedUser.xpProximo || 1000);
            updatedUser.xpProximo = Math.floor((updatedUser.xpProximo || 1000) * 1.5);
            alert(`🎉 PARABÉNS! Você subiu para o nível ${updatedUser.nivel}!`);
        }
